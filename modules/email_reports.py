...
def get_email_config():
    """Pobiera konfigurację email — najpierw z DB config, fallback na JSON file"""
    try:
        ...
        if host:
            return {
                ...
                'email': get_config('smtp_user', ''),
                'password': get_config('smtp_password', ''),
                'recipient': get_config('admin_email', ''),
                'enabled': bool(host)
            }
    except:
        pass
...
<body>
    <div class="container">
        ...
        <div class="content">
            ...
            <!-- Alert o zwrotach -->
            {f'<div class="alert">⚠️ <strong>{week_stats["zwroty"]} zwrotów</strong> w tym tygodniu</div>' if week_stats['zwroty'] > 0 else ''}
