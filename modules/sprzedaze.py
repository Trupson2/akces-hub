...
<div class="quick-actions" style="grid-template-columns:repeat(4,1fr);margin-bottom:16px">
    ...
    <a href="/sprzedaze/napraw-nazwy?miesiac={{ miesiac_filter }}" class="qa-btn">
        <span class="qa-icon" style="background:rgba(143,245,255,0.12)">🔧</span>
        Napraw dane
    </a>
...
<div class="list-item" style="{% if s.is_zwrot %}opacity:0.5{% endif %}">
    ...
    <div class="list-item-right">
        <div class="list-item-value" style="color:{% if s.is_zwrot %}var(--red){% else %}#beee00{% endif %}">
...
<div class="grupa-item card" style="padding:14px">
    ...
    {% if g.suggestion %}
    <div style="background:rgba(190,238,0,0.12);border:1px solid rgba(190,238,0,0.3);border-radius:8px;padding:8px;margin-top:8px;display:flex;align-items:center;gap:8px">
        ...
        <div style="flex:1;min-width:0">
            <div style="font-size:0.75rem;color:#beee00">Sugestia:</div>
...
<div id="searchModal" style="display:none;position:fixed;top:0;left:0;right:0;bottom:0;background:rgba(0,0,0,0.85);z-index:1000;padding:15px;overflow-y:auto">
    <div class="card" style="max-width:500px;margin:40px auto;padding:20px;background:rgba(15,15,30,0.85);backdrop-filter:blur(16px);border:1px solid rgba(255,255,255,0.08)">
        <div class="card-header">
            <div class="card-title" style="color:#8ff5ff;font-family:'Space Grotesk',sans-serif;text-shadow:0 0 20px rgba(143,245,255,0.3)">🔍 Szukaj produktu</div>
            ...
            if (d.ok) {
                ...
                items.forEach(el => {
                    if (el.innerHTML.includes(saleIds.split(',')[0])) {
                        ...
                        el.innerHTML = '<div style="text-align:center;color:#beee00;padding:10px">&#10003; Dopasowano ' + d.matched + ' szt. &rarr; ' + d.product_name.substring(0,40) + '</div>';
                    }
                });
            } else {
                alert('Blad: ' + d.msg);
