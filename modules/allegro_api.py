...
def index():
    ...
    revenue_brutto = conn.execute("SELECT COALESCE(SUM(cena*ilosc),0) as s FROM sprzedaze WHERE strftime('%Y-%m',data_sprzedazy)=? AND status NOT IN ('zwrot','anulowane','anulowana')", (_month,)).fetchone()['s']
    ...
    revenue_month = revenue_brutto - zwroty_suma  # przychód netto (po odliczeniu zwrotów)
