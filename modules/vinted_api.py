...
def _save_cookies(cookies_dict):
    ...
    try:
        ...
        print(f"[OK] Vinted cookies saved ({len(cookies_dict)} cookies)")
    except Exception as e:
        print(f"[ERR] Vinted cookies save error: {e}")
...
def _load_cookies():
    ...
    try:
        ...
        print(f"[WARN] Vinted cookies load error: {e}")
    return {}
...
def _get_csrf_token(session):
    ...
    try:
        ...
        if resp.status_code == 200:
            ...
            if match:
                return match.group(1)
        print(f"[WARN] CSRF token not found (status: {resp.status_code})")
    except Exception as e:
        print(f"[ERR] CSRF token error: {e}")
    return None
    ...
    for attempt in range(MAX_RETRIES):
        try:
            ...
            if resp.status_code == 429:
                ...
                print(f"[WARN] Vinted rate limit, waiting {wait}s...")
                time.sleep(wait)
                continue
