    ...
    <style>
        ...
        <div id="panel-dzis" class="stat-panel">
            <div class="card" style="background:var(--green-soft);border-color:rgba(34,197,94,0.3)">
                <div style="color:var(--green);font-weight:600;font-size:1.1rem;margin-bottom:12px"><span class=material-symbols-outlined style=font-size:1rem;vertical-align:middle>calendar_month</span> DZIS ({datetime.now().strftime('%d.%m.%Y')})</div>
                <div class="stat-row">
                    <div class="stat-box">
        ...
        <div id="panel-alltime" class="stat-panel" style="display:none">
            <div class="card">
                <div style="color:var(--orange);font-weight:600;font-size:1.1rem;margin-bottom:12px"><span class=material-symbols-outlined style=font-size:1rem;vertical-align:middle>trending_up</span> LACZNIE (ALL-TIME)</div>
                <div style="display:grid;grid-template-columns:repeat(2,1fr);gap:12px">
                    <div class="stat-box" style="text-align:center">
        ...
        <div class="card">
            <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:12px">
                <div style="color:var(--purple);font-weight:600;font-size:1.1rem"><span class=material-symbols-outlined style=font-size:1rem;vertical-align:middle>bar_chart</span> WYKRES ({current_year})</div>
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:10px;margin-top:20px;margin-bottom:10px">
            ...
            <a href="/analityka" class="btn btn-purple" style="display:flex;align-items:center;justify-content:center;gap:6px"><span class=material-symbols-outlined style=font-size:1rem;vertical-align:middle>trending_up</span> Analityka</a>
        </div>
    ...
    <style>
        ...
        <div class="card" style="margin-bottom:20px">
            <div style="display:flex;gap:10px;flex-wrap:wrap;align-items:center">
                ...
                <button class="btn btn-sm btn-purple" onclick="bulkUpdate()"><span class=material-symbols-outlined style=font-size:1rem;vertical-align:middle>edit_note</span> Zastosuj</button>
                <span style="color:var(--text-muted)">|</span>
                <button class="btn btn-sm btn-success" onclick="autoKategoryzuj()"><span class=material-symbols-outlined style=font-size:1rem;vertical-align:middle>smart_toy</span> Auto-kategoryzuj wszystkie</button>
            </div>
        </div>
...
def analityka_okazje():
    ...
    live_scraper_section = """
        <div class='card' style='border-color:rgba(14,165,233,0.25);margin-bottom:20px'>
          ...
          <div id='szukaj-panel' style='background:var(--bg);border:1px solid rgba(34,197,94,0.2);border-radius:10px;padding:14px'>
            <div style='display:flex;align-items:center;justify-content:space-between;margin-bottom:10px'>
              <div style='color:var(--green);font-weight:600;font-size:0.9rem'><span class=material-symbols-outlined style=font-size:1rem;vertical-align:middle>shopping_cart</span> Szukaj palet pod mój profil (AI)</div>
    ...
    if not has_trendy:
        ...
        <div class='card' style='border-color:rgba(59,130,246,0.25);margin-bottom:20px'>
          <div style='color:var(--blue);font-weight:700;font-size:1rem;margin-bottom:12px'><span class=material-symbols-outlined style=font-size:1rem;vertical-align:middle>smart_toy</span> Analiza rynkowa (Perplexity AI)</div>
        ...
        def _cache_block(odp, cits, ts, refresh_url, title, icon):
            ...
            def _make_link_btn(m):
                ...
                return f'<div style="margin:6px 0"><a href="{url}" target="_blank" style="display:inline-block;background:var(--blue);color:#fff;padding:8px 18px;border-radius:8px;text-decoration:none;font-weight:700;font-size:0.85rem"><span class=material-symbols-outlined style=font-size:1rem;vertical-align:middle>link</span> {label} ↗</a> <span style="color:var(--text-muted);font-size:0.73rem">{rest}</span></div>'
        ...
        <div class='card' style='border-color:rgba(139,92,246,0.25);margin-bottom:20px'>
          <div style='display:flex;align-items:center;justify-content:space-between;gap:12px;flex-wrap:wrap;margin-bottom:10px'>
            <div style='color:var(--purple);font-weight:700;font-size:1rem'><span class=material-symbols-outlined style=font-size:1rem;vertical-align:middle>smart_toy</span> Perplexity AI</div>
            <div style='display:flex;align-items:center;gap:8px'>
              <form method='POST' action='/analityka/okazje/set-perplexity-model' style='margin:0;display:flex;align-items:center;gap:6px'>
                ...
                <select name='model' onchange='this.form.submit()' class='form-control' style='width:auto;padding:3px 8px;font-size:0.75rem;cursor:pointer'>
                  <option value='sonar-pro' {{'selected' if perplexity_model in ("sonar","sonar-pro") else ""}}>Sonar Pro <span class=material-symbols-outlined style=font-size:1rem;vertical-align:middle>star</span> (zalecany)</option>
              ...
              <form method='POST' action='/analityka/okazje/remove-perplexity-key' onsubmit="return confirm('Usunąć klucz Perplexity?')" style='margin:0'>
                <input type='hidden' name='csrf_token' value='{generate_csrf()}'>
                <button type='submit' style='background:transparent;color:var(--text-muted);border:none;cursor:pointer;font-size:0.8rem'><span class=material-symbols-outlined style=font-size:1rem;vertical-align:middle>delete</span> usuń klucz</button>
              </form>
            </div>
          </div>
          </div>
          ...
          <div class='card' style='border-color:rgba(139,92,246,0.2)'>
              <div style='color:var(--purple);font-weight:600;font-size:0.85rem;margin-bottom:4px'><span class=material-symbols-outlined style=font-size:1rem;vertical-align:middle>bar_chart</span> Analiza moich sprzedaży</div>
              ...
              <form method='POST' action='/analityka/okazje/perplexity-analyze' onsubmit='showLoading(this,"analyze")'>
                ...
                <button id='btn-analyze' type='submit' class='btn btn-purple' style='border:none;cursor:pointer;font-size:0.82rem'>
                  <span class=material-symbols-outlined style=font-size:1rem;vertical-align:middle>search</span> Analizuj moje produkty
                </button>
              </form>
    ...
    no_data_banner = ""
    if not has_trendy:
        no_data_banner = "<div class='alert alert-warning'><span class=material-symbols-outlined style=font-size:1rem;vertical-align:middle>warning</span> Brak danych — uruchom <code style='background:var(--bg);padding:2px 6px;border-radius:4px'>python analyze_trends.py</code></div>"
...
<div>
  <div class="section-title"><span class=material-symbols-outlined style=font-size:1rem;vertical-align:middle>bar_chart</span> Wszystkie produkty (top 50)</div>
  {wszystkie_html}
</div>
    ...
    fetch('/analityka/okazje/perplexity-status?klucz=' + klucz)
      ...
      .then(d => {{
        if(d.status === 'done') {{
          ...
          clearInterval(pollInterval);
          document.getElementById('loading-' + loadingParam).innerHTML = '<span class=material-symbols-outlined style=font-size:1rem;vertical-align:middle>cancel</span> Błąd Perplexity — sprawdź klucz API i spróbuj ponownie';
        }}
      }});
  }}, 3000);
...
def scrape_jobalots():
    ...
    try:
        ...
        for item in items:
            ...
            tag_label = {'popularity': '<span class=material-symbols-outlined>local_fire_department</span>', 'most_bids': '<span class=material-symbols-outlined style=font-size:1rem;vertical-align:middle>trending_up</span>', 'bid_low': '[PAID]'}.get(sort_tag, '')
            products.append({
                'title': title,
        ...
        return jsonify({'ok': True, 'products': products, 'total': total, 'source': 'api',
            'note': f'<span class=material-symbols-outlined>local_fire_department</span> Popularne · <span class=material-symbols-outlined style=font-size:1rem;vertical-align:middle>trending_up</span> Dużo ofert · [PAID] Najtańsze ({total} palet łącznie)'})
    except Exception as e:
...
def analityka_czas_sprzedazy():
    ...
    if stat_w:
        ...
        <div style="margin-top:8px;display:flex;gap:6px;flex-wrap:wrap">
            <span class="badge badge-success"><span class=material-symbols-outlined style=font-size:1rem;vertical-align:middle>bolt</span> {stat_w['w_24h']} w 24h</span>
            <span style="background:var(--blue-soft);color:var(--blue);padding:3px 8px;border-radius:10px;font-size:0.7rem;font-weight:600"><span class=material-symbols-outlined style=font-size:1rem;vertical-align:middle>calendar_month</span> {stat_w['w_7dni']} w tyg</span>
    ...
    if najszybsze:
        ...
        szybkie_html = f'<div class="card"><div style="font-weight:700;color:var(--green);margin-bottom:12px"><span class=material-symbols-outlined style=font-size:1rem;vertical-align:middle>bolt</span> Najszybciej sprzedane (od dodania do systemu)</div>{rows}</div>'
    ...
    if dw:
        chart_html = f"""
        <div class="card">
            <div style="font-weight:700;color:var(--text-secondary);margin-bottom:12px"><span class=material-symbols-outlined style=font-size:1rem;vertical-align:middle>bar_chart</span> Rozkład czasu sprzedaży (od dodania do systemu)</div>
    ...
    <div class="dash-grid" style="margin-bottom:20px">
        <div class="card" style="border-color:rgba(34,197,94,0.4)">
            <div style="font-weight:700;color:var(--green);margin-bottom:12px;display:flex;align-items:center;gap:8px"><span class=material-symbols-outlined style=font-size:1rem;vertical-align:middle>list_alt</span> Od DODANIA DO SYSTEMU</div>
            {karta_w}
        </div>
        <div class="card" style="border-color:rgba(59,130,246,0.4)">
            <div style="font-weight:700;color:var(--blue);margin-bottom:12px;display:flex;align-items:center;gap:8px"><span class=material-symbols-outlined style=font-size:1rem;vertical-align:middle>local_shipping</span> Od ZAKUPU PALETY</div>
            {karta_z}
        </div>
    </div>
    {chart_html}
    ...
    <div class="card" style="border-color:rgba(59,130,246,0.25);display:flex;align-items:center;justify-content:space-between;gap:12px;flex-wrap:wrap">
      <div>
        <div style='color:var(--blue);font-weight:600;font-size:0.85rem'><span class=material-symbols-outlined style=font-size:1rem;vertical-align:middle>calendar_month</span> Daty wystawienia ofert</div>
      ...
      <a href='/allegro/sync-oferty-daty' class='btn btn-sm btn-primary' style='text-decoration:none;white-space:nowrap'><span class=material-symbols-outlined style=font-size:1rem;vertical-align:middle>sync</span> Odśwież daty z Allegro</a>
    </div>
...
def _run_pallet_analysis(job_id, paleta_id, api_key, db_path, model="gemini-2.0-flash", excel_products=None, provider="gemini"):
    ...
    try:
        ...
        for batch_idx, batch in enumerate(batches):
            ...
            _pallet_analysis_jobs[job_id] = {
                'status': 'running',
                'progress': f'<span class=material-symbols-outlined style=font-size:1rem;vertical-align:middle>check_circle</span> Batch {batch_idx+1}/{len(batches)} gotowy ({pct}%) — {len(all_results)} produktów przeanalizowanych'
            }
        ...
        try:
            ...
            _pallet_analysis_jobs[job_id] = {
                'status': 'running',
                'progress': f'<span class=material-symbols-outlined style=font-size:1rem;vertical-align:middle>search</span> Weryfikuję ceny na Allegro (0/{len(all_results)})...'
            }
            allegro_verified = 0
            allegro_corrected = 0
            for ri, r in enumerate(all_results):
                ...
                _pallet_analysis_jobs[job_id] = {
                    'status': 'running',
                    'progress': f'<span class=material-symbols-outlined style=font-size:1rem;vertical-align:middle>search</span> Weryfikuję ceny na Allegro ({ri+1}/{len(all_results)})... <span class=material-symbols-outlined style=font-size:1rem;vertical-align:middle>check_circle</span> {allegro_verified} zweryfikowanych, <span class=material-symbols-outlined style=font-size:1rem;vertical-align:middle>sync</span> {allegro_corrected} skorygowanych',
...
def _get_render_results_js():
    ...
    function renderResults(d) {
        ...
        if (parsed && parsed.produkty && parsed.produkty.length) {
            ...
            html += '<input type="text" id="product-filter" placeholder="<span class=material-symbols-outlined style=font-size:1rem;vertical-align:middle>search</span> Filtruj np. peruka, wig, hair..." style="flex:1;min-width:200px;padding:8px 12px;background:rgba(0,0,0,0.3);border:1px solid var(--border);border-radius:8px;color:var(--text);font-size:0.85rem">';
            ...
            html += '<select id="product-sort" onchange="sortProducts()" style="padding:8px 12px;background:rgba(0,0,0,0.3);border:1px solid var(--border);border-radius:8px;color:var(--text);font-size:0.85rem"><option value="">Sortuj...</option><option value="popyt"><span class=material-symbols-outlined>local_fire_department</span> Popyt (wysoki→niski)</option><option value="cena_desc">[PAID] Cena (najdroższe)</option><option value="cena_asc">[PAID] Cena (najtańsze)</option><option value="wartosc"><span class=material-symbols-outlined style=font-size:1rem;vertical-align:middle>bar_chart</span> Wartość (najwyższa)</option><option value="czas"><span class=material-symbols-outlined>timer</span> Czas sprzedaży (najszybsze)</option></select>';
    ...
    function renderProductTable(products) {
        ...
        products.forEach(function(p, idx) {
            ...
            var asinLink = '';
            if (p.asin) {
                asinLink = '<br><a href="https://www.amazon.de/dp/' + p.asin + '" target="_blank" style="font-size:0.7rem;color:#60a5fa;text-decoration:none"><span class=material-symbols-outlined style=font-size:1rem;vertical-align:middle>link</span> ' + p.asin + '</a>';
            } else if (p.ean) {
    ...
    function filterProducts() {
        ...
        sumEl.innerHTML = '<div style="display:flex;gap:20px;flex-wrap:wrap;align-items:center">'
            + '<span style="font-weight:700;color:var(--green)"><span class=material-symbols-outlined style=font-size:1rem;vertical-align:middle>search</span> "' + q + '"</span>'
