...
def produkt_extract_params(produkt_id):
    ...
    if not GEMINI_CLIENT:
        content = f'''
        <div class="header">
            <h1>[WARNING] Extraktor Allegro</h1>
    ...
    content = f'''
    <div class="header">
        <h1>[SMART_TOY] Extraktor Allegro</h1>
        <small>Produkt #{produkt_id}</small>
    </div>
    ...
    <div style="background:var(--green-soft);border:2px solid rgba(34,197,94,0.5);border-radius:12px;padding:20px;margin-bottom:20px">
        <div class="section-title">[EDIT_NOTE] META TYTUŁ ALLEGRO (Skopiuj poniżej)</div>
    ...
    <div class="section-title">[SETTINGS] PARAMETRY TECHNICZNE</div>
...
def produkt_quick_draft(produkt_id):
    ...
    try:
        ...
        if result and 'id' in result:
            ...
            content = f'''
            <div class="header">
                <h1>[CHECK_CIRCLE] Szkic utworzony!</h1>
                <small>Produkt #{produkt_id}</small>
            </div>
...
def paleta_edit(paleta_id):
    ...
    <form method="POST" class="card">
        ...
        <div class="form-group">
            <label>[PIN_DROP] Regal / Lokalizacja</label>
...
def napraw_ceny_palet():
    ...
    <div style="text-align:center;padding:60px 20px">
        <div style="font-size:3rem;margin-bottom:20px">[CHECK_CIRCLE]</div>
...
def palety_lista():
        ...
        <div class="paleta-card" id="paleta-card-{p['id']}">
            <div style="display:flex;justify-content:space-between;align-items:start">
                <div style="display:flex;align-items:start;gap:8px">
                    ...
                    <div>
                        ...
                        {f'<div style="font-size:0.75rem;color:var(--purple);margin-top:2px">[PIN_DROP] Regal: {regal}</div>' if regal else ''}
                    </div>
                </div>
            ...
            <div style="margin-top:10px;background:var(--bg);border-radius:6px;padding:8px">
                <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:6px">
                    <span style="font-size:0.75rem;color:var(--text-secondary)">[BAR_CHART] Sprzedano:</span>
    ...
    <div id="bulk-delete-bar" style="display:none;position:sticky;top:60px;z-index:50;background:linear-gradient(135deg,var(--red),#dc2626);border-radius:10px;padding:12px 16px;margin-bottom:12px;display:none;align-items:center;justify-content:space-between;box-shadow:0 4px 15px rgba(239,68,68,0.4)">
        ...
        <button type="button" onclick="bulkDeletePalety()" style="background:#fff;color:var(--red);border:none;padding:8px 20px;border-radius:8px;font-weight:700;cursor:pointer;font-size:0.85rem">[DELETE] USUŃ ZAZNACZONE</button>
    </div>
...
def paleta_dodaj():
    ...
    <a href="/palety/import-xlsx" class="import-link" style="background:linear-gradient(135deg,var(--green),#16a34a)">
        <div class="import-link-inner">
            <div class="import-link-icon">[BAR_CHART]</div>
            <div>
    ...
    <form method="POST" class="card">
        ...
        <button type="submit" class="btn btn-success">[SAVE] ZAPISZ PALETĘ</button>
    </form>
...
def paleta_import_xlsx():
    ...
    error_html = ''
    if error:
        error_html = f'<div class="alert alert-error" style="margin-bottom:15px">[WARNING] Błąd: {error}</div>'

    content = f'''
    <div class="header">
        <h1>[BAR_CHART] IMPORT Z EXCEL</h1>
    ...
    <form method="POST" enctype="multipart/form-data" class="card">
        ...
        <!-- MAPOWANIE KOLUMN -->
        <div class="section-title" style="margin-top:20px">[LINK] MAPOWANIE KOLUMN</div>
...
def paleta_bulk_import():
    ...
    if request.method == 'POST':
        try:
            ...
            # [LIST_ALT] Dodaj produkty z ASIN do tabeli scraped (żeby pojawiły się w generatorze ofert)
            scrape_count = 0
            try:
            ...
            for w in wyniki:
                if w['status'] == 'merged':
                    ...
                    typ_icon = '[INBOX]' if w.get('typ') == 'box' else '[CHECK_CIRCLE]'
                    results_html += f'''
                    ...
                    <div style="display:flex;align-items:center;gap:10px;padding:12px;background:var(--red-soft);border:1px solid rgba(239,68,68,0.3);border-radius:10px;margin-bottom:8px">
                        <div style="font-size:1.5rem">[CANCEL]</div>
                        <div style="flex:1">
                            <div style="font-weight:600">{w['nazwa']}</div>
            ...
            content = f'''
            <div class="header">
                <h1>[BAR_CHART] WYNIKI IMPORTU</h1>
            ...
            <a href="/palety/bulk-import" style="display:block;text-align:center;color:var(--text-muted);text-decoration:none;margin-top:10px">[BAR_CHART] Importuj kolejne</a>
            '''
    ...
    # === GET - formularz ===
    error = request.args.get('error', '')
    error_html = f'<div class="alert alert-error" style="margin-bottom:15px">[WARNING] {error}</div>' if error else ''

    content = f'''
    <div class="header">
        <h1>[BAR_CHART] BULK IMPORT PALET</h1>
    ...
    <div class="card" style="margin-bottom:15px">
        <div class="section-title">[SETTINGS] WSPÓLNE USTAWIENIA</div>
        ...
        <div style="margin-top:15px;padding:12px;background:var(--bg);border-radius:10px;border:1px solid var(--border)">
            <div style="font-size:0.8rem;font-weight:600;color:var(--green);margin-bottom:6px">[SMART_TOY] AUTO-DETEKCJA KOLUMN</div>
...
def paleta_mass_edit(paleta_id):
    ...
    if not produkty or len(produkty) == 0:
        content = f'''
        <div class="header">
            <h1>[WARNING] Brak produktów</h1>
            <small>Paleta #{paleta_id}</small>
        </div>
        <div class="alert alert-warning">
            Ta paleta nie ma jeszcze żadnych produktów. Najpierw zaimportuj produkty z Excel.
        </div>
        <a href="/magazyn/import?paleta_id={paleta_id}" class="btn btn-primary">[DOWNLOAD] Importuj produkty</a>
    ...
    for p in produkty:
        ...
        if p['status'] == 'wystawiony':
            status_badge = '<span class="badge badge-success">[CHECK_CIRCLE] WYSTAWIONE</span>'
            ...
            status_badge = '<span class="badge" style="background:var(--accent-soft);color:var(--purple)">[EDIT_NOTE] SZKIC</span>'
        ...
        <div class="product-row" style="background:{row_bg}">
            ...
            <div style="display:flex;align-items:center;gap:8px;margin-bottom:8px;padding:8px 10px;background:var(--green-soft);border-radius:8px">
                <div style="font-size:0.85rem;color:var(--green);flex:1;min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;font-weight:600">[EDIT_NOTE] {str(p["meta_title"])[:80]}</div>
            ...
            <div style="display:flex;align-items:center;gap:8px;margin-bottom:8px;padding:8px 10px;background:var(--red-soft);border-radius:8px">
                <div style="font-size:0.85rem;color:var(--red);flex:1;font-weight:600">[WARNING] Brak META TITLE</div>
                <button onclick="regenerateMetaTitle({p["id"]}, this)"
    ...
    function batchGenerateMetaTitles() {{
        ...
        if (!confirm('[SMART_TOY] Wygenerować META TITLE dla ' + checked.length + ' produktów?\\n\\n<span class=material-symbols-outlined>timer</span>  Szacowany czas: ~' + timeStr + '\\n[WARNING]  5s opóźnienie między produktami (safe rate limiting)\\n\\nKontynuować?')) {{
            return;
        }}
...
def paleta_szczegoly(paleta_id):
    ...
    for p in produkty:
        ...
        status_opcje = ''
        statusy = [('magazyn','[INVENTORY_2] Magazyn'),('wystawiony','[SHOPPING_CART] Wystawiony'),('sprzedany','[PAID] Sprzedany'),('uszkodzony','[WARNING] Uszkodzony'),('zwrot','[UNDO] Zwrot')]
        ...
        <div style="background:var(--bg-card);border:1px solid var(--border);border-radius:8px;padding:10px;margin-bottom:8px" data-produkt-id="{p['id']}" data-ilosc="{p['ilosc']}">
            ...
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:6px;margin-bottom:8px">
                <div>
                    <div style="font-size:0.6rem;color:var(--text-muted);margin-bottom:3px">[LABEL] STAN</div>
    ...
    <div class="header">
        ...
        {f'<div style="margin-top:6px;font-size:0.85rem;color:var(--purple)">[PIN_DROP] Regal: {regal_palety}</div>' if regal_palety else ''}
    </div>

    <!-- GŁÓWNE STATYSTYKI SPRZEDAŻY -->
    <div class="sale-banner">
        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:10px">
            <div class="sale-banner-label">[BAR_CHART] SPRZEDAŻ Z PALETY</div>
    ...
    <div class="stat-row" style="margin-bottom:15px">
        <div class="stat-box" style="background:var(--green-soft);border-color:rgba(34,197,94,0.3)">
            <div class="stat-val green">[CHECK_CIRCLE] {stats['wystawione'] or 0}</div>
        ...
        <div class="stat-box" style="background:var(--red-soft);border-color:rgba(239,68,68,0.3)">
            <div class="stat-val red">[BAR_CHART] {roi:.1f}%</div>
            <div class="stat-lbl">ROI</div>
        </div>
    </div>

    <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:10px;margin-bottom:10px">
        <a href="/palety/{paleta_id}/mass-edit" class="btn btn-purple" style="text-decoration:none">[EDIT] MASOWE WYSTAWIANIE</a>
        <a href="/magazyn/import?paleta_id={paleta_id}" class="btn" style="background:var(--blue);text-decoration:none">[DOWNLOAD] IMPORTUJ EXCEL</a>
        <a href="/palety/{paleta_id}/edit" class="btn btn-warning" style="text-decoration:none">[SETTINGS] EDYTUJ PALETE</a>
    </div>
    <button type="button" id="scrape-btn" data-paleta="{paleta_id}" class="btn" style="width:100%;background:linear-gradient(135deg,#8b5cf6,#6d28d9);margin-bottom:15px;font-size:0.85rem;cursor:pointer">[PHOTO_CAMERA] SCRAPUJ ZDJECIA ({bez_zdjec} produktow bez zdjec)</button>
    ''' + '''<script>
    document.getElementById('scrape-btn').onclick = function() {
        ...
        .then(function(d) {
            if (d.ok) {
                btn.innerHTML = '[CHECK_CIRCLE] Scraping ' + d.count + ' produktow w tle!';
                btn.style.background = '#22c55e';
                btn.style.opacity = '1';
            } else {
                btn.innerHTML = '[CANCEL] ' + (d.error || 'Blad');
    ...
    <form method="POST" action="/palety/''' + str(paleta_id) + '''/delete" style="margin-top:20px" onsubmit="return confirm('[WARNING] UWAGA!\\n\\nTo usunie tę paletę i wszystkie jej produkty (''' + str(stats['cnt']) + ''' szt.)\\n\\nNa pewno kontynuować?')">
        <input type="hidden" name="csrf_token" value="''' + generate_csrf() + '''">
    ...
    <div id="modalKorekta" onclick="if(event.target===this)this.style.display='none'" class="modal-overlay" style="display:none">
        <div class="modal-box" style="max-width:400px;margin:50px auto">
            <h3 style="margin:0 0 15px">[EDIT] Korekta produktu</h3>
            ...
            <form method="POST" action="/sprzedaze/korekta-ilosci">
                ...
                <div style="display:flex;gap:10px;margin-bottom:15px">
                    <button type="submit" class="btn" style="flex:1;background:var(--blue);margin:0">[SAVE] Zapisz ilość</button>
            ...
            <div id="cofnijOfflineSection" style="display:none;border-top:1px solid var(--border);padding-top:15px;margin-top:15px">
                <label style="display:block;font-size:0.8rem;color:var(--red);margin-bottom:8px">[SYNC] Cofnij sprzedaż offline:</label>
                ...
                <div style="display:flex;gap:10px">
                    ...
                    <button onclick="cofnijOffline()" class="btn btn-danger" style="width:auto;padding:12px 20px;margin:0">[SYNC] Cofnij</button>
                </div>
            </div>

            <!-- COFNIJ SPRZEDAŻ -->
            <div style="border-top:1px solid var(--border);padding-top:15px;margin-top:15px">
                <label style="display:block;font-size:0.8rem;color:var(--red);margin-bottom:8px">[SYNC] Cofnij sprzedaż (przywróć do magazynu):</label>
                <button onclick="cofnijSprzedaz()" class="btn btn-danger" style="margin:0">[SYNC] Cofnij sprzedaż</button>
                <div style="font-size:0.65rem;color:var(--text-muted);margin-top:6px;text-align:center">
                ...
                html += '<div style="background:var(--yellow-soft);border:1px solid rgba(245,158,11,0.3);border-radius:10px;padding:12px;margin-bottom:8px">' +
                  '<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px">' +
                    ...
                    '<div style="display:flex;gap:6px">' +
                      '<button onclick="edytujNaprawa('+s.id+', \''+((s.opis_naprawy||'').replace(/'/g,"\\'"))+'\')" style="padding:4px 10px;background:var(--purple);border:none;border-radius:6px;color:#fff;font-size:0.72rem;cursor:pointer">[EDIT] Edytuj</button>' +
                    ...
                    '</div>' +
                  '</div>' +
                  '<div style="background:var(--bg-card);border-radius:6px;padding:8px;font-size:0.8rem">[EDIT_NOTE] '+(s.opis_naprawy || '—')+'</div>' +
    ...
    function pokazMenu(evt, produktId, ilosc, nazwa) {
        ...
        document.getElementById('menuStatusy').innerHTML =
            '<div onclick="menuStatus(\'sprzedany\')" class="menu-item">[CHECK_CIRCLE] Sprzedane</div>' +
            '<div onclick="menuStatus(\'sprzedany_uszkodzony\')" class="menu-item">[WARNING] Sprzedane uszkodzone</div>' +
            '<div onclick="menuNaprawyModal('+produktId+', \''+nazwa.replace(/'/g,"\\'")+'\', '+ilosc+')" class="menu-item">[BUILD] Do naprawy...</div>' +
            '<div onclick="menuStatus(\'wyrzucenie\')" class="menu-item">[DELETE] Do wyrzucenia</div>' +
        ...
        document.getElementById('menuInne').innerHTML =
            ...
            '<a href="/magazyn/produkt/'+produktId+'/edytuj" class="menu-item" style="text-decoration:none;display:block;color:var(--text)">[EDIT] Edytuj produkt</a>';
