...
def serwis_lista():
    ...
    html += f'''
        <a href="/serwis?status=przyjety" class="sv-stat-card sv-stat-cyan">
            <div class="sv-stat-icon">[DOWNLOAD]</div>
        ...
        <a href="/serwis?status=naprawiony" class="sv-stat-card sv-stat-lime">
            <div class="sv-stat-icon">[CHECK_CIRCLE]</div>
        ...
        <a href="/serwis?status=zwrocony" class="sv-stat-card sv-stat-outline">
            <div class="sv-stat-icon">[SYNC]</div>
        ...
        <a href="/serwis?status=zlomowany" class="sv-stat-card sv-stat-secondary">
            <div class="sv-stat-icon">[DELETE]</div>
    ...
    <div class="sv-filters">
        ...
        <a href="/serwis?status=przyjety" class="sv-filter-pill ''' + ('active' if filtr=='przyjety' else '') + '''">[DOWNLOAD] Przyjęte</a>
        <a href="/serwis?status=w_naprawie" class="sv-filter-pill ''' + ('active' if filtr=='w_naprawie' else '') + '''">[BUILD] W naprawie</a>
        <a href="/serwis?status=naprawiony" class="sv-filter-pill ''' + ('active' if filtr=='naprawiony' else '') + '''">[CHECK_CIRCLE] Naprawione</a>
        <a href="/serwis?status=zwrocony" class="sv-filter-pill ''' + ('active' if filtr=='zwrocony' else '') + '''">[SYNC] Zwrócone</a>
        <a href="/serwis?status=zlomowany" class="sv-filter-pill ''' + ('active' if filtr=='zlomowany' else '') + '''">[DELETE] Złomowane</a>
    </div>
        ...
        for item in items:
            ...
            <div class="sv-item {border_cls}">
                ...
                <div style="flex:1;min-width:0">
                    ...
                    <div class="sv-item-fault">[BOLT] {item['opis_usterki'] or 'Brak opisu usterki'}</div>
                </div>
            ...
            if item['status'] == 'przyjety':
                html += f'''
                    ...
                    <button onclick="zlomuj({item['id']})" class="sv-act sv-act-scrap">[DELETE]</button>
                '''
            elif item['status'] == 'w_naprawie':
                html += f'''
                    <button onclick="zakonczNaprawe({item['id']})" class="sv-act sv-act-done">[CHECK_CIRCLE] Naprawiony</button>
                    <button onclick="zlomuj({item['id']})" class="sv-act sv-act-scrap">[DELETE]</button>
                '''
            elif item['status'] == 'naprawiony':
                html += f'''
                    <button onclick="zwrocDoMagazynu({item['id']})" class="sv-act sv-act-return">[SYNC] Zwróć do mag.</button>
                '''
